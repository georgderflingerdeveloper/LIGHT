    ...
    class SchedulerDataRecovery
    {
        ...
        public void RecoverScheduler( string directory, string device )
        {
            ...
            if( RecoveredData != null )
            {
                ...
                if( ERecovered != null )
                {
                    ERecovered( null, EventArgs.Empty );
                }
            }
        } 
        ...
        void DetectDataChange( FeedData feed )
        {
            ...
            Store( _directory, feed, GotNewJob );
        }
        ...
        private List<FeedData> Recover( string directory, string device )
        {
            ...
            catch( Exception ex )
            {
                ...
                // fake data recovered to allow new saving
                if( ERecovered != null )
                {
                    ERecovered( null, EventArgs.Empty );
                }
        ...
        public  void Store( string directory, FeedData data, bool GotNewJob )
        {
            try
            {
                if( SchedFeedData != null )
                {
                    ...
                    XmlSerializer ser = new XmlSerializer( typeof( List<FeedData> ) );
                    string currentDir = Environment.CurrentDirectory;
                    FileStream str = new FileStream( @directory + "\\" + data.Device + FileExtensions.StoredDataExtension, FileMode.Create );
